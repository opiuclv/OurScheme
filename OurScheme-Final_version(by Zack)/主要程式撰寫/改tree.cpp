...
typedef enum {LEFT_PAREN = 1, RIGHT_PAREN = 2, INT = 3, STRING = 4, DOT = 5, FLOAT = 6,
NIL = 7, T = 8, QUOTE = 9, SYMBOL = 10} Token_enum;
...
Tree_node * BuildTree( Tree_node *ptr, Tree_node *exp  ) { // 建樹 龍骨一直是空的 遞迴是回上一層的進入點 
	...
	if ( strcmp(gToken[gNum].data, "(") != 0 && strcmp(gToken[gNum].data, ")") != 0 ) { 
		...
		else if ( gNum == 0 ) strcpy(ptr->tree_token.data, gToken[gNum].data); 	
...
void GetToken () { // GetToken 先全部照單全收，要印的時候用C語言把歸類成 float的token，用%3f的函數去印 
	...
	while ( temp != EOF ) { // 讀到結束
		...
		int k = 0 ;
		while ( k <= gToken.size()-1 && gToken.size() != 0 ) {
		...
		if ( ch[0] == '\n' || ch[0] == EOF ) {
			...
			gNum = 0 ;
			if( gToken.size() != 0 ) PrintTree(root) ;
		} // if
		// *************************************************處理parse tree ************************************* 
		strcpy( text, "") ;
		if( ch[0] == EOF || ch[0] == '\n') gToken.clear() ;
